...
public class MongodbGuardiumFilter implements Filter {
    ...
    public Collection<Event> filter(Collection<Event> events, FilterMatchListener matchListener) {
        if (log == null) { 
            ...
            log = LogManager.getLogger(MongodbGuardiumFilter.class); 
        ...
        for (Event e : events) {
            log.debug("MongoDB filter: Got Event: "+logEvent(e)); // won't show when deployed on Guardium collector.
    ...
    private int getAuditMsgStartIndex(String messageString){
        ...
        if (mongodIndex>=0){
            log.debug("This is mongoD message"); // won't show when deployed on Guardium collector.
            return mongodIndex;
        }
        mongodIndex = messageString.indexOf(MONGOS_AUDIT_START_SIGNAL);
        if (mongodIndex>=0){
            log.debug("This is mongoS message"); // won't show when deployed on Guardium collector.
            return mongodIndex;
        }
        log.error("Message start index was not found and message will be ignored");
        return -1;
    }
